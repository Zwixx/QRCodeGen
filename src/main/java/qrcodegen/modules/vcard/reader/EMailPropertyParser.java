 ...
 */
public class EMailPropertyParser extends PropertyParser {
	...
	void parse() {
		try {
			...
			String eMail = VCardTools.deEscape(value);
			try {
				EMailProperty.Builder builder = new EMailProperty.Builder(eMail);
	...
	@Override
	EMailProperty getPropertyEntry() {
		if (!isValid()) {
